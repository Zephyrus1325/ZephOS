...
int32_t k_svc_dispatcher(uint32_t id, uint32_t arg1, uint32_t arg2, uint32_t arg3) {
    switch (id) {
        ...
        case SYS_PRINTF:
            ...
            //k_enable_interrupts();
            //k_uart_print("Syscall PRINTF chamada!\r\n"); // Teste de vida
            //k_uart_print((char*)arg1); // Imprime apenas a string de formato
            k_uart_print("\n\r[DEBUG] Syscall ID: ");
            k_uart_print_hex(id);
            k_uart_print("\n\r[DEBUG] Arg1 (addr): 0x");
            k_uart_print_hex(arg1); // Veja se o endereço faz sentido (ex: 0x8000...)
            
            if (arg1 < 0x100) { // Endereço muito baixo é erro de ponteiro NULL
                k_uart_print("\n\r[DEBUG] Erro: Ponteiro Arg1 invalido!");
            } else {
                k_uart_print("\n\r[DEBUG] Char em Arg1: ");
                k_uart_putc(*(char*)arg1); // Tenta imprimir apenas o primeiro caractere
                k_uart_print("\n\r[DEBUG] String completa: ");
                k_uart_print((char*)arg1);
            }
